...
/* 
 ...
 Updated for Megatronics V3.2 Board by Matt Henriksen
 Edited by Blaine Harris & Adrian Gonzalez
 Latest version (V0.11) 2/15/2019
 
 TAMU Update:
  - The screen and control board problems with the Megatronics 3.2 Board have been fixed have been fixed. -M@
  - Added Texas A&M boot screen
  - Fixed problem with plan-B code not being interpreted by the printer
  - Fixed "No SD card" message not displaying
  - Increased values (x4) for home speed to slow homing down (may have been reverted)
  - Inverted the motor directions to the build plate and feed plate 2
...
/*
 ...
 Hardware:
 * 74HC165 shift input register atached to pins 35 (42 M@), 34 (35 M@) and 43 (33 M@) of arduino, as detailed below.
 * the encoder also attached on the keypad is attached to pins 45 and 44 (34 M@). The printer should be able to function without encoder.
...
const int latchPin = 42;  // 35 changed to 42 - M@
// Pin connected to data pin (CP) of 74HC165
const int dataPin = 35; // 34 changed to 35 - M@
// Pin connected to clock pin (  ) of 74HC165
const int clockPin = 33;  // 43 changed to 33 - M@

//input encoder
const int encoderPin1 = 45;
const int encoderPin2 = 34; // 44 changed to 34
